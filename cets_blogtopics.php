...
Plugin Name: cets_blogtopics

Plugin URI:

Description: WordPressMU plugin for classifying blogs into topics. 

Version: 1.2

Author: Deanna Schneider (http://deannaschneider.wordpress.com)

Thanks goes out to Ezequiel Conte (http://code.google.com/p/cetsopensource/) for getting me started.
...
/* *******************************************************************************
 * Get all Blogs that are part of a specific topic id
 * *******************************************************************************
 */
 ...
 */   
 ...
 /* ********************************************************************************
 * Gets a select box of all topics formatted for Blog Topics and Blog Edit pages
 * ********************************************************************************
 */   
   ...
   // Adds or updates the blog topic, whichever is necessary
	 ...
	 function get_topics_table()
    {
    	...
    	foreach ( $this->get_topics() as $topic )  {
			...
			echo("<td  valign='top'><input type='hidden' name='action' value='edit'> <input type='submit' name='edit' value='Edit'> </td>");
			echo("</form><td valign='top'>");
			if ($topic->id != 1) {
				echo "<form name='deletecat' method='post'><input type='hidden' name='action' value='delete'><input type='hidden' name='topicid' value='" . $topic->id . "'><input type='submit' value='Delete'></form>";
			}
			else {
    	...
    	echo("<tr><form name='catadd' method='post'><td>&nbsp;</td><td><input type='text' name='topic' value=''><input type='hidden' name='action' value='add'></td>");
		echo("<td><input type='text' name='slug' value=''></td>");
		echo("<td><textarea name='description' cols='40' rows='5'></textarea>");
		echo("<td><input type='submit' value='Add'></form></td></tr></table>");
    }
	...
	function add_siteadmin_page(){
      if (is_site_admin()) {
      	...
      	if (function_exists('is_network_admin')) {
      		...
      		add_submenu_page('settings.php', 'Blog Topics Management', 'Blog Topics Management', 10, 'cets_bc_management_page', array(&$this, 'cets_bc_management_page'));
      		...
      		add_submenu_page('ms-admin.php', 'Blog Topics Management', 'Blog Topics Management', 10, 'cets_bc_management_page', array(&$this, 'cets_bc_management_page'));
	 ...
	 function cets_bc_management_page(){
        ...
        <h2>Update/Add Blog Topics</h2>
			...
			<div>
				...
				$this->get_topics_select_featured($featured); ?> <input type="submit" value="Set Featured Topic">
